...
public final class PickServerActivity extends PreferenceActivity implements
		PortSweeper.Callback, DialogInterface.OnClickListener {
    
    private static final String TAG = "PickServerActivity";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mPreferenceWiFi = (CheckBoxPreference) preferenceScreen
				.findPreference(KEY_WIFI);
		mProgressCategory = (ProgressCategory) preferenceScreen
				.findPreference(KEY_SERVERS);
	...
	private void forget(String server) {
		...
		// Send the updated list of remembered servers even if the activity is canceled
		Intent data = new Intent();
		data.putStringArrayListExtra(EXTRA_REMEMBERED, mRemembered);
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
		...
		if (preference != null) {
			String server = preference.getTitle().toString();
			if (mRemembered.contains(server)) {
				menu.add(Menu.NONE, CONTEXT_FORGET, Menu.NONE,
						R.string.context_forget);
			}
		}
	}
